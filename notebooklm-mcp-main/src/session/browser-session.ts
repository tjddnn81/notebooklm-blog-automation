...
export class BrowserSession {
  ...
  private async waitForNotebookLMReady(): Promise<void> {
    ...
    // Try multiple selectors in order (language-agnostic as much as possible)
    const chatInputSelectors = [
      "textarea.query-box-input",                     // Primary class-based
      "textarea[aria-label*='query' i]",              // aria-label contains 'query' (case-insensitive)
      "textarea[aria-label*='쿼리' i]",               // Korean
      "textarea[placeholder*='Ask' i]",               // English placeholder
      "textarea[placeholder*='질문' i]",              // Korean placeholder
      "textarea:not([disabled])",                     // Any non-disabled textarea (generic fallback)
    ];
    ...
    for (const selector of chatInputSelectors) {
      try {
        ...
        log.success(`  ✅ Chat input ready with: ${selector}`);
        return;
      } catch {
        continue;
      }
    ...
    // If all fail, give specific error
    log.error(`  ❌ NotebookLM interface not ready: Could not find chat input`);
    throw new Error(
      "Could not find NotebookLM chat input. " +
      "Please ensure the notebook page has loaded correctly and a source has been added."
    );
  }
