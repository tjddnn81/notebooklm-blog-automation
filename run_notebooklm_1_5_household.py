...
def main():
    blogs = [
        {
            "topic": "'원룸은 좁고 투룸은 비싸고? 1.5 가구가 쏘아 올린 2026 부동산·비즈니스 지각변동",
            "source": "source_trend_2_1_5_household.txt",
            "output": "Trend_Blog_2_1_5_Household_Spicy.md"
        }
    ]
    ...
    for blog_info in blogs:
        ...
        try:
            ...
            notebook = client.create_notebook("트렌드: 1.5 가구")
            ...
            deep_search_query = f"""
            당신은 상위 0.1%의 경제/부동산/테크 분야 전문 인사이트 블로거이자, 구독자들의 뼈를 때리는 '팩트폭행' 1타 강사입니다.
            블로그 이름은 'Stay Log'입니다.
            이번 글의 목적은 뻔한 부동산 글에 지친 독자들에게 '도파민'을 터뜨려주면서도 팩트와 인사이트를 뇌리에 꽂아넣는 것입니다.
            ...
            [🚨 유튜브형 초강력 도파민 작성 규칙 🚨]
            1. 멱살 잡는 서론 (Hooking): "아직도 아파트 영끌만 쳐다보고 계신가요? 벼락거지 확정입니다." 등, 유튜브 썸네일 급의 강력한 어그로로 확 끌어당기며 시작하세요.
            2. 뼈 때리는 팩트 폭행과 사이다 말투: 딱딱한 경제 잡지 말투를 버리고, '슈카월드' 채널 패널처럼 친근하고 확신에 찬 구어체(~해요, ~죠, 팩트를 말씀드릴게요 등)를 100% 사용하세요.
            3. 찰떡 비유 (스토리텔링): 어려운 경제/부동산 용어는 일상적인 내용(연애, 당근마켓, 원룸 자취 등)으로 비유해서 쉽고 찰지게 설명하세요.
            4. 시각적 도파민 마사지: 단락마다 🚀, 🔥, 🏠, 🚨, 💸, 🤑, 😎 등의 이모지를 팍팍 넣어서 스크롤의 지루함을 박살 내세요.
            5. 기승전결과 명확한 핵심 전략: 재미만 있는 게 아니라, 결국 "그래서 우리는 어디에 투자하거나 어떻게 대응해야 하는데?"에 대해 코리빙 비즈니스나 관련 타겟 섹터를 콕 집어주면서 마무리하세요.
            
            형식은 완벽한 Markdown으로 출력해야 합니다. 독자가 번쩍 깰 정도로 어그로 끌면서도 흡입력 있고 유익하게 작성하세요!
            """
            try:
